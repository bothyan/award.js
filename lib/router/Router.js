...
export default class Router extends React.Component {
    ...
    constructor(props, context) {
        ...
        this.beforeLoading = this.beforeLoading.bind(this)
    ...
    async beforeLoading({ to, query, route }) {
        const { before } = this.props

        if (!!before && typeof before === 'function') {
            return await before({ url: to, query })
        }
    ...
    async afterLoading({ Component, route, query, to, history = true, push = true, replace = false }) {
        ...
        if (!!after && typeof after === 'function') {
            change = await after({ Component, query, url: to })
        }
    ...
    getChildContext() {
        return {
            AWARD_PROPS: {
                ...
                Award_Loading: {
                    before: this.beforeLoading,
                    loading: this.loading,
                    after: this.afterLoading
                }
            }
        }
    }
