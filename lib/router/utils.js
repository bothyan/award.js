...
export async function LinkPage(e = null) {
    ...
    if (/http(s)?:\/\//.test(to)) {
        ...
        if (Award_Loading != null) {
            //这里可以设置路由的生命周期
            beforeLoading = await Award_Loading.before({ to, query, route })
        }
        ...
        if (beforeLoading) {
            ...
            //执行文件执行
            if (Award_Loading != null) {
                Award_Loading.after({ Component, route, query, to })
            } else {
                let callback = () => {
...
export async function popstate(main = true) { 
    if (main) {
        ...
        const { page: route, query } = ResolveRouter(routes)
        const to = location.href
        const beforeLoading = await this.beforeLoading({ to, query, route })

        //加载之前的逻辑返回true，可以执行加载动作了
        if (beforeLoading) {
            await this.loading()
            const Component = await routeLoader.loadPage(route)
            this.afterLoading({ Component, route, query, to, history:false })
        }
