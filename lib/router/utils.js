...
export async function LinkPage(e = null) {
    ...
    if (/http(s)?:\/\//.test(to)) {
        ...
        if (Award_Loading != null) {
            //这里可以设置路由的生命周期
            beforeLoading = await Award_Loading.before({ to, query, route, name, error })
        }
        ...
        if (beforeLoading) {
            ...
            //执行文件执行
            if (Award_Loading != null) {
                Award_Loading.after({ Component, route, query, to, name, error })
            } else {
                let callback = () => {
                ...
                routeLoader.notify({ Component, route, query, callback, to, error })
            }
        }
    }
}
...
export async function popstate(main = true) { 
    if (main) {
        ...
        const beforeLoading = await this.before({ to, query, route, name, error })

        //加载之前的逻辑返回true，可以执行加载动作了
        if (beforeLoading) {
            await this.loading()
            const Component = await routeLoader.loadPage(route)
            this.after({ Component, route, query, to, name, history:false, error })
        }
        ...
        const to = location.href
        const Component = await routeLoader.loadPage(route)
        routeLoader.notify({ Component, route, query, to, error })        
    }    
}

export function changeHistory({ push = true, replace = false, to }) { 
