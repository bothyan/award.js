...
export default class Loader { 
    ...
    constructor(assetPrefix) { 
        ...
        this.hasloadedRoutes = []
    }

    // 加载资源进行判断区分
    loadPage(route) {
        const error = !!route === false    
        route = error ? '/_award_error' : route
    ...
    loadSource(route,error) { 
        ...
        script.src = `${this.assetPrefix}${error ? '/static' : ''}${route}`
    ...
    update(data) {  
        ...
        if (this.hasloadedRoutes.indexOf(data.route) != -1) {            
            this.notify(data)
        } else { 
            this.hasloadedRoutes.push(data.route)
        }        
    }
